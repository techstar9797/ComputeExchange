...
_shared_types_path = str(Path(__file__).parent.parent.parent / "packages" / "shared-types")
if _shared_types_path not in sys.path:
    sys.path.insert(0, _shared_types_path)

# Import from shared-types models module
# Using importlib to avoid name collision with local modules
import importlib.util
spec = importlib.util.spec_from_file_location(
    "shared_models",
    Path(__file__).parent.parent.parent / "packages" / "shared-types" / "models.py"
)
_shared_models = importlib.util.module_from_spec(spec)
...
spec.loader.exec_module(_shared_models)
